        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-6">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white tracking-tight leading-none mb-1">{activeToolData.name}</h2>
                <p className="text-xs text-white/40 font-medium tracking-tight">HackBook Offensive Intelligence / {activeToolData.category}</p>
              </div>
            </div>
          </div>
        ...
        <div className="relative min-h-[500px] overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#0a1117]/96 p-3 shadow-[0_24px_80px_rgba(0,0,0,0.32)] sm:p-4">
          ...
          {(() => {
            switch (activeToolId) {
              ...
              default: return <div className="text-white/20 italic">Tool not found: {activeToolId}</div>;
            }
          })()}
        </div>
      </div>
      ...
      <header className="flex flex-wrap items-center justify-center gap-2.5 border-b border-white/5 pb-6">
        {toolCategories.map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)}
            className={`px-4 py-1.5 text-[11px] font-bold tracking-tight rounded-md transition-all border ${activeCategory === cat ? 'bg-[#9fef00]/10 border-[#9fef00]/30 text-[#9fef00] shadow-[0_0_20px_rgba(159,239,0,0.05)]' : 'bg-white/5 border-white/5 text-white/40 hover:border-white/20 hover:text-white'}`}>
            {cat}
          </button>
        ))}
      </header>
        ...
        {filteredTools.map((tool) => (
          <div key={tool.id} onClick={() => handleSelectTool(tool.id)} className="htb-card p-6 group cursor-pointer hover:translate-y-[-2px] transition-all flex flex-col h-full relative overflow-hidden">
            ...
            <div className="space-y-2.5 flex-grow">
              <h3 className="text-base font-bold text-white tracking-tight group-hover:text-[#9fef00] transition-colors leading-tight">{tool.name}</h3>
              <p className="text-[12px] text-white/50 leading-relaxed font-medium tracking-tight">{tool.description}</p>
            </div>
            <div className="mt-6 pt-4 border-t border-white/5 flex flex-wrap gap-1.5">
              {tool.tags.map(tag => (
                <span key={tag} className="text-[8px] font-bold text-white/20 uppercase tracking-widest group-hover:text-white/40 transition-colors">#{tag}</span>
              ))}
            </div>
          </div>
        ))}
