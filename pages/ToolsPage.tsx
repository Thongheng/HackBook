      ...
      <header className="flex flex-wrap items-center justify-center gap-2.5 border-b border-white/5 pb-6">
        {toolCategories.map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)}
            className={`px-4 py-1.5 text-[11px] font-bold tracking-tight rounded-md transition-all border ${activeCategory === cat ? 'bg-[#9fef00]/10 border-[#9fef00]/30 text-[#9fef00] shadow-[0_0_20px_rgba(159,239,0,0.05)]' : 'bg-white/5 border-white/5 htb-text-faint hover:border-white/20 hover:htb-text-muted'}`}>
            {cat}
          </button>
        ))}
      </header>
        ...
        {filteredTools.map((tool) => (
          <div key={tool.id} onClick={() => handleSelectTool(tool.id)} className="htb-card p-6 group cursor-pointer hover:translate-y-[-2px] transition-all flex flex-col h-full relative overflow-hidden">
            <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center mb-5 border border-white/5 group-hover:border-[#9fef00]/20 transition-colors">
              <tool.icon className="w-5 h-5 htb-text-faint group-hover:text-[#9fef00] transition-colors" />
            </div>
            <div className="space-y-2.5 flex-grow">
              <h3 className="text-base font-bold htb-text tracking-tight group-hover:text-[#9fef00] transition-colors leading-tight">{tool.name}</h3>
