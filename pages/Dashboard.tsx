...
import {
  ...
  Terminal, Cpu, ClipboardList, ExternalLink, Trash2,
...
const TOOL_ICONS: Record<string, React.FC<any>> = {
  ...
  'tool-phpfilter': Cpu, 'tool-msfvenom': Terminal, 'tool-xss': Globe,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg mx-4 bg-[#0b1217] border border-white/10 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-white/5">
          <h3 className="text-sm font-bold text-white tracking-tight">Pin to Favorites</h3>
        ...
        <div className="px-5 pt-4 pb-2">
          <div className="relative">
            ...
            <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search..."
              className="w-full bg-white/[0.03] border border-white/5 rounded-lg pl-8 pr-3 py-2 text-sm text-white/70 placeholder-white/20 focus:outline-none focus:ring-1 focus:ring-[#9fef00]/30" />
          </div>
        </div>
        <div className="flex border-b border-white/5 px-5">
          {(['tools', 'guides', 'refs'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-2.5 text-[10px] font-bold tracking-[0.15em] uppercase border-b-2 transition-colors -mb-px ${tab === t ? 'border-[#9fef00] text-[#9fef00]' : 'border-transparent text-white/30 hover:text-white/60'}`}>
          ...
          {tab === 'tools' && filteredTools.map(tool => {
            ...
            return (
              <button key={tool.id} disabled={faved} onClick={() => { onAdd({ kind: 'tool', id: tool.id, name: tool.name, category: tool.category }); }}
                ...
                <span className="text-[12px] text-white/70 font-medium">{tool.name}</span>
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded border ${CAT_COLORS[tool.category] || 'text-white/30 bg-white/5 border-white/10'}`}>{faved ? 'Pinned' : tool.category}</span>
              </button>
            );
          })}
          {tab === 'guides' && filteredGuides.map(g => {
            ...
            return (
              <button key={g.idx} disabled={faved} onClick={() => { onAdd({ kind: 'guide', idx: g.idx, title: g.title, category: g.category }); }}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all ${faved ? 'opacity-30 cursor-default' : 'hover:bg-white/[0.04] cursor-pointer'}`}>
                <span className="text-[12px] text-white/70 font-medium">{g.title}</span>
          ...
          {tab === 'refs' && filteredRefs.map(r => {
            ...
            return (
              <button key={r.url} disabled={faved} onClick={() => { onAdd({ kind: 'ref', url: r.url, title: r.title, category: r.category }); }}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all ${faved ? 'opacity-30 cursor-default' : 'hover:bg-white/[0.04] cursor-pointer'}`}>
                <span className="text-[12px] text-white/70 font-medium">{r.title}</span>
...
const FavCard: React.FC<{ item: FavItem; onRemove: () => void; onClick: () => void }> = ({ item, onRemove, onClick }) => {
  ...
  if (item.kind === 'tool') {
    ...
    iconColor = CAT_COLORS[item.category] || 'text-white/40 bg-white/5 border-white/10';
  ...
  return (
    <div onClick={onClick}
      ...
      <button
        ...
        className="absolute top-3 right-3 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 hover:text-red-400 text-white/30 transition-all z-10">
      ...
      <div className="space-y-2 flex-grow">
        <h3 className="text-base font-bold text-white tracking-tight group-hover:text-[#9fef00] transition-colors leading-tight">{label}</h3>
        <p className="text-[12px] text-white/40 font-medium tracking-tight capitalize">{sub}</p>
      </div>

      <div className="mt-5 pt-4 border-t border-white/5 flex items-center justify-between">
        <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest group-hover:text-white/40 transition-colors">#{tagLabel}</span>
        {item.kind === 'ref' && <ExternalLink className="w-3 h-3 text-white/20 group-hover:text-[#9fef00]/50 transition-colors" />}
      </div>
    </div>
  );
};
      ...
      <section className="space-y-5 animate-in fade-in duration-700">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight">
            Welcome <span className="text-white/30">Hacker</span>
          </h1>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-white tracking-tight">Pinned Favorites</h2>
            {favs.length > 0 && (
              <span className="text-[10px] font-bold text-white/30 bg-white/5 border border-white/5 px-2 py-0.5 rounded-full">{favs.length}</span>
            )}
          </div>
          <button onClick={() => setShowPicker(true)}
            className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 text-white/50 font-bold rounded-lg hover:bg-white/10 hover:text-white transition-all text-[11px] uppercase tracking-widest">
        ...
        {favs.length === 0 ? (
          <div onClick={() => setShowPicker(true)}
            ...
            <div className="w-14 h-14 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-center group-hover:border-[#9fef00]/20 transition-colors">
              <Star className="w-6 h-6 text-white/15 group-hover:text-[#9fef00]/40 transition-colors" />
            </div>
            <div className="text-center space-y-1">
              <p className="text-sm font-bold text-white/30">No favorites pinned yet</p>
              <p className="text-[12px] text-white/20">Pin tools, guides, or references for fast access during engagements.</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            <div onClick={() => setShowPicker(true)}
              ...
              <div className="w-10 h-10 rounded-lg bg-white/[0.02] border border-white/5 flex items-center justify-center group-hover:border-[#9fef00]/20 transition-colors">
                <Plus className="w-5 h-5 text-white/20 group-hover:text-[#9fef00]/50 transition-colors" />
              </div>
              <span className="text-[10px] font-bold text-white/20 group-hover:text-white/40 uppercase tracking-widest transition-colors">Add Item</span>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-5 pb-8">
        {[
          ...
          <div key={i} onClick={f.action}
            ...
            <div className="pt-1 flex items-center gap-1.5 text-[10px] font-bold text-white/20 group-hover:text-white/50 uppercase tracking-widest transition-colors">
